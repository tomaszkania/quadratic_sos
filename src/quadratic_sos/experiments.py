...
from quadratic_sos.enumeration import _exact_row_interval, enumerate_search_set, is_representable, relevant_squares
...
def search_profile(order: RealQuadraticOrder, alpha: Pair, *, balance: bool = True) -> SearchProfile:
    ...
    # Construct them only after the cheap counts above.
    t2 = set()
    for idx, s1 in enumerate(squares):
        for s2 in squares[idx:]:
...
def brute_force_exact_length(order: RealQuadraticOrder, alpha: Pair) -> Optional[int]:
    """Compute the exact length by an intentionally naïve search.

    The routine is designed only for validation on modest trace ranges.  It does
    not use the package's exact row-search engine.
    """
    ...
    for s in squares:
        if order.sub(alpha, s) in t2:
            return 3
    if p == 4:
        return 4
    for t in t2:
        if order.sub(alpha, t) in t2:
            return 4
    return 5
    ...
    for D in Ds:
        ...
        for alpha in enumerate_totally_positive(order, trace_bound):
            ...
            if exact_length(order, alpha) != brute_force_exact_length(order, alpha):
                mismatches += 1
    return ValidationSummary(field_count=len(Ds), trace_bound=trace_bound, element_count=elements, mismatches=mismatches)
